...
/* ─── Green particle dots + connections for Worker module ─── */
...
const GreenParticleCanvas = () => {
  ...
  const DOT_COLOR = "80, 200, 120"; // green dots
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        ctx.fillStyle = `rgba(${DOT_COLOR}, ${(p.opacity + glowBoost) * 0.7})`;
        ctx.fill();

        for (let j = i + 1; j < particles.length; j++) {
          const p2 = particles[j];
          const lx = p.x - p2.x;
          if (Math.abs(lx) > CONNECTION_DIST) continue;
          const ly = p.y - p2.y;
          ...
          if (ldSq < CONNECTION_DIST * CONNECTION_DIST) {
            ...
            const lineOpacity = 0.12 * (1 - ld / CONNECTION_DIST) * 0.7;
            ctx.beginPath();
            ctx.moveTo(p.x, p.y);
...
export default GreenParticleCanvas;
